...
public class InteractionFileLoader{
    ...
    HashMap<Integer, Group> addedGroups = new HashMap<Integer, Group>();
        ...
        //loads interaction data into memory from Document
    public InteractionFileLoader(Document conDoc, Interaction interaction, ModelFileChecker mc){
    ...
    //load the document into memory
     Document doc = conDoc;
        ...
        for(int i=0; i < gList.getLength(); i++){
            ...
            newGroup.setName(group.getElementsByTagName("name").item(0).getTextContent());
            ...
            for(int j=0;j<mList.getLength();j++){
                ...
                String microName = micro.getElementsByTagName("name").item(0).getTextContent();
                ...
                (new Decoder(mc, false)).readMicrointeraction(microFile, microFile.getAbsolutePath(), newMicro); //maybe should use true for deecoder
                            ...
                            if(glob.getName().equals(globName)){
                                if(glob.getType().equals("array")){
                                    ...
                                    for(int r = 0; r < items.getLength(); r++){
                                        ...
                                        glob.addItemToArray(item.getAttribute("value"), item.getAttribute("link"));
                                    }
                                }else{
        ...
        for(int i=0;i<tList.getLength(); i++){
            ...
            groupTransition = new GroupTransition(addedGroups.get(sourceID), addedGroups.get(targetID), null);
    ...
    public InteractionFileLoader(String fileName, Interaction interaction, ModelFileChecker mc){
