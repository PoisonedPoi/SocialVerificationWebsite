...
public class ConcurrentChecker {
  ...
  public void check(Prism prism, PrismLog mainLog) {
    ...
    try {
      modulesFile =
        prism.parseModelFile(
          new File(
            c.getUSERFOLDER() +
            "prism" +
            File.separator +
            group.getMacrointeraction().getName() +
            ".pm"
          )
        );
    ...
    for (Microinteraction micro : group.getMicrointeractions()) {
      for (Module mod : micro.getModules()) {
        for (State st : mod.getStates()) {
          if (
            st.getStateClass().equals(StateClass.SPEAKING) &&
            st.getAgent().equals("human")
          ) {
            h_speech = true;
            String property = "E [ F (Speechhuman) ];";
            try {
              propertiesFile =
                prism.parsePropertiesString(modulesFile, property);
              result =
                prism.modelCheck(
                  propertiesFile,
    ...
    if (r_speech_reachable && !h_speech_reachable) {
      ...
      String propertyHuman =
        "Pmax=? [ ( !(Speechhuman | Speechrobot)) U Speechhuman ];";
      String propertyRobot =
        "Pmax=? [ ( !(Speechhuman | Speechrobot)) U Speechrobot ];";
      try {
        propertiesFile =
          prism.parsePropertiesString(modulesFile, propertyHuman);
        result =
        ...
        if (resultStr.contains("1.0")) {
          group.setHSpeaksFirst(true);
        } else group.setHSpeaksFirst(false);

        propertiesFile =
          prism.parsePropertiesString(modulesFile, propertyRobot);
        result =
          prism.modelCheck(propertiesFile, propertiesFile.getPropertyObject(0));
        ...
        if (resultStr.contains("1.0")) {
          group.setSpeaksFirst(true);
        } else group.setSpeaksFirst(false);
      } catch (PrismLangException e) {
        // TODO Auto-generated catch block
        e.printStackTrace();
      } catch (PrismException e) {
    ...
    if (h_speech && r_speech) {
      ...
      try {
        ...
        PrintWriter writer = new PrintWriter(
          new File(c.getUSERFOLDER() + "tempout.txt")
        );
        writer.print("");
        ...
        result =
          prism.modelCheck(propertiesFile, propertiesFile.getPropertyObject(0));
        mainLog.flush();
        if (result.toString().equals("false")) aggregateResults = false;
      } catch (PrismLangException e) {
        // TODO Auto-generated catch block
        e.printStackTrace();
    ...
    for (Microinteraction micro : group.getMicrointeractions()) {
      if (
        micro.getName().equals("Greeter") ||
        micro.getName().equals("Farewell") ||
        micro.getName().equals("Instruction") ||
        micro.getName().equals("Remark") ||
        micro.getName().equals("Answer") ||
        micro.getName().equals("Ask")
      ) {
        ...
        for (Microinteraction hmicro : group.getMicrointeractions()) {
          if (!hmicro.equals(micro)) { // can't look at the same micro!
            if (
              hmicro.getName().equals("Answer") ||
              hmicro.getName().equals("Ask") ||
              hmicro.getName().equals("Instruction") ||
              hmicro.getName().equals("Greeter") ||
              hmicro.getName().equals("Farewell") ||
              (
                hmicro.getName().equals("Wait") &&
                hmicro.findVariable("allow_speech").getValue().equals("true")
              )
            ) {
    ...
    for (Module mod : ma.getModules()) {
      ...
      for (Microinteraction mi : group.getMicrointeractions()) {
        if (mod.getName().equals(mi.getName())) {
          HashMap<Integer, ArrayList<State>> idx2states = micro2idx2states.get(
            mi
          );
          ...
          while (it.hasNext()) {
            ...
            for (State st : states) {
              if (
                !st.getGaze().equals("GAZE_ELSE") &&
                !st.getGaze().equals("GAZE_AT") &&
                !st.getGaze().equals("NONE")
              ) {
                gazeStates.put(idx, st.getGaze());
              }
              if (
                !st.getGesture().equals("GESTURE_NONE") &&
                !st.getGesture().equals("NONE")
              ) {
    ...
    ArrayList<PropModsBeh> gazeProperties = getAllProperties(
      combinations,
      moduleGazeStates,
      ma
    );
    ArrayList<PropModsBeh> gestureProperties = getAllProperties(
      combinations,
      moduleGestureStates,
      ma
    );
    ...
    for (PropModsBeh prop : gazeProperties) {
      ...
      if (values.size() == 1) propsToRemove.add(prop);
    }
    for (PropModsBeh prop : propsToRemove) gazeProperties.remove(prop);
    for (PropModsBeh prop : gestureProperties) {
      HashMap<Module, String> mod2beh = prop.mod2beh;
      Set<String> values = new HashSet<String>(mod2beh.values());
      if (values.size() == 1) propsToRemove.add(prop);
    }
    for (PropModsBeh prop : propsToRemove) gestureProperties.remove(prop);

    // now perform the checking of each property!
    group.wipeUnresolvedBehConflicts();
    for (PropModsBeh pmb : gazeProperties) {
      Result rawResult = runProperty(
        prism,
        mainLog,
        pmb.property,
        modulesFile,
      ...
      if (rawResult.toString().equals("false")) group.checkIfViolating(
        pmb,
        false
    ...
    /*
		...
		for (ModBehPair mbp : group.getUnresolvedBehaviorConflicts()) {
			...
			if (!inPrevBadMBP && !mbp.getNonProperty()) {
				...
				System.out.println("concurrentCheck 378 add bug");
			}
		}	
  ...
  private void generateProperties(
    Microinteraction ma,
    ArrayList<Module> combo,
    HashMap<Module, ArrayList<Integer>> stateIndices,
    HashMap<Module, Integer> currIdxs,
    ArrayList<PropModsBeh> properties,
    HashMap<Module, HashMap<Integer, String>> moduleBehStates
  ) {
    // base case
    boolean done = false;
    ...
    for (int i = 0; i < combo.size(); i++) {
      ...
      property +=
        moduleStateStrs.get(mod) +
        "=" +
        stateIndices.get(mod).get(currIdxs.get(mod)) +
        " ";
      if (i != combo.size() - 1) property += " & ";

      modBeh.put(
        mod,
        moduleBehStates
          .get(mod)
          .get(stateIndices.get(mod).get(currIdxs.get(mod)))
      );
    }
  ...
  private void findCombinations(
    int[] idxs,
    int r,
    ArrayList<ArrayList<Module>> combinations,
  ...
  private ArrayList<PropModsBeh> getAllProperties(
    ArrayList<ArrayList<Module>> combinations,
    HashMap<Module, HashMap<Integer, String>> moduleBehStates,
    Microinteraction ma
  ) {
    ...
    for (ArrayList<Module> combo : combinations) {
      ...
      for (Module mod : combo) currIdxs.put(mod, 0);
