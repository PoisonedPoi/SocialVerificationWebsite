...
public class Conflict extends TreeItem<HBox> {
	...
	public Conflict(Property prop, String describe, String type, ArrayList<String> choices, ModBehPair mbp, Group group, ModelFileChecker mc, Image helperImage, Image icon) {
		if(prop == null){
			...
			for (Fix fix : prop.getFixes()) {
				...
				fixer.setOnAction(new EventHandler<ActionEvent>() {
		            ...
		            public void handle(ActionEvent event) {
		            	Platform.runLater(
		    					() -> {
		    		               //mc.repairInteraction(fix.getID(), group, false);  //do we do this???????????????????????????????????????????/
		    					}
		    				);}
		        });
