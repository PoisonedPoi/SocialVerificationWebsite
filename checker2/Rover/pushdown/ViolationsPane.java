...
public class ViolationsPane {
	...
	public ViolationsPane(TreeView conflictBox, Interaction ia, ArrayList<String> propertyCategories, ModelFileChecker mc) {
		this.ia = ia;
		this.mc = mc;
		...
		conflictBox.setStyle("-fx-background-color: blue;");
	...
	public void update() {
		System.out.println("*************** violation pane update called *** *************");
		...
		if (ia.getInit().getMicrointeractions().isEmpty()) {
			...
			for (Group group : ia.getGroups()) {
				...
				if (group.getViolating()) {
					System.out.println("	Group is violating");
				...
				System.out.println("	checking conflict group, init");
				// add conflicts!
				...
				for (Property prop : ia.getGraphProperties()) {
					...
					if (prop.getTies().equals("group") || prop.getTies().equals("init")) { //check all group specific properties
						...
						System.out.println("		property was violated");
	...
	public void addGazeGestureConflicts(Group group) {
		System.out.println(" *********** calling violations pane **********");
