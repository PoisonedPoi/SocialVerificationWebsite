...
#defaults
prismLoc="prism-library"  #/opt/resources/prism-library     /srv/rover/lib/prism-library
...
export LD_LIBRARY_PATH=${prismLoc}/prism/lib/ #required as some .so files in prism have other .so dependencies, so this is needed as well as setting the java library path
